...
namespace GiyimMagazaOtomasyonu
{
    class Fonksiyonlar
    {
        int personelId; //sisteme bağlanan personel ıd si
        ...
        public int Kullanici_giris(string kAdi, string kSifre){
            if (this.mysql_baglan() == true)
            {
                ...
                {//tablo okunurken denk gelen kısma göre fonksiyon geri döner 
                    ...
                    if(oku["KullaniciAdi"].ToString() == kAdi && oku["Sifre"].ToString() == kSifre)
                    {
                        personelId = Int32.Parse(oku["PersonelId"].ToString());
                    }
        ...
        public DataTable UrunSat(string urunBarkod)
        {
            ...
            if (this.mysql_baglan() == true)
            {
                ...
                data = new MySqlDataAdapter(sorgu, baglan);
                data.SelectCommand.Parameters.AddWithValue("?p_barkod", Int32.Parse(urunBarkod));
                tablo = new DataTable();
                data.Fill(tablo);
                foreach (DataRow row in tablo.Rows)
                {
            ...
            if (this.mysql_baglan() == true)
            {
                string sorgu = "select * from personel where MagazaId=" + personelId + "";
                ...
                foreach (DataRow row in tablo.Rows)
                {
                    string magaza = row["MagazaId"].ToString();
                    magazaId = Int32.Parse(magaza);
                }
            }
            ...
            if (this.mysql_baglan() == true)
            {
                cmd = new MySqlCommand("insert into satis(SatisAdedi, SatisTarihi, UrunId, MagazaId, PersonelId) VALUES(1,1995-10-10,"+urunId+","+magazaId+","+personelId+")", baglan);
               if (cmd.ExecuteNonQuery() == 1)
                {
                    this.mysql_bitir();
                    return tablo;
